 ...
 *
 * 仅 win32 生效；其余平台直接跳过（mpv --focus-on-open 已自带）。
 */
...
function bringToFront(pid) {
    ...
    try {
        ...
        const child = spawn(
            'powershell.exe',
            ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-Command', HELPER, String(pid)],
            { stdio: 'ignore', windowsHide: true, detached: true }
        );
        // PowerShell 启动较慢，这里用退避：先等一两个周期再 spawn，窗口大多已创建
        child.unref();
    } catch (e) { /* 辅助进程失败不影响播放 */ }
}
