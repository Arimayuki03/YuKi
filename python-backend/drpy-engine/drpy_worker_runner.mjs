/**
 * drpy_worker_runner.mjs
 * 正式 drpy Node.js 沙箱隔离运行器，支持 drpy 规则执行与 JSON-RPC 2.0 stdio IPC 通信。
 * 
 * 核心特性：
 * 1. 完整注入 drpy 宿主能力：pdfa/pdfh/pdft/pd, cheerio, CryptoJS, dayjs, req/request/post, local, joinUrl 等
 * 2. 严格沙箱隔离：
 *    - 阻止规则 require('child_process')
 *    - 阻止规则访问任意文件系统，仅允许 Worker 专属受控临时目录
 *    - 阻断 Object.constructor 逃逸面
 * 3. 健壮的生命周期与错误捕获
 */
...
// 1. 安全受控临时目录与受控 fs
// ----------------------------------------------------------------------------
// 支持从环境变量指定或自动创建专用临时目录
const BASE_TEMP_DIR = process.env.VPC_WORKER_TEMP_DIR || path.join(os.tmpdir(), 'drpy_worker_' + process.pid);
...
function ensureSafePath(targetPath) {
    if (!targetPath) {
        throw new Error('[SecurityError] Path cannot be empty.');
    }
    const resolved = path.resolve(BASE_TEMP_DIR, String(targetPath));
    if (!resolved.startsWith(BASE_TEMP_DIR)) {
...
// ----------------------------------------------------------------------------
// 3. drpy 同步网络请求 (通过宿主托管的 sync_http 子进程)
// ----------------------------------------------------------------------------
...
class DrpyWorkerContainer {
    ...
    initContext() {
        const local = {
            get: (k1, k2) => {
                if (k2 !== undefined) return this.localStore.get(`${k1}:${k2}`) || '';
                return this.localStore.get(String(k1)) || '';
            },
            set: (k1, k2, v) => {
                if (v !== undefined) {
                    this.localStore.set(`${k1}:${k2}`, String(v));
                } else {
                    this.localStore.set(String(k1), String(k2));
                }
            },
            delete: (k1, k2) => {
                if (k2 !== undefined) this.localStore.delete(`${k1}:${k2}`);
                else this.localStore.delete(String(k1));
            },
        ...
        const sandbox = {
            ...
            // 日志重定向到 stderr (不污染 stdout JSON-RPC)
            console: {
    ...
    loadRule(ruleSource) {
        ...
        // 统一 ESM / CJS / __jsEvalReturn 模块包装
...
// ----------------------------------------------------------------------------
// 5. JSON-RPC 2.0 stdio 消息循环
// ----------------------------------------------------------------------------
const worker = new DrpyWorkerContainer();
