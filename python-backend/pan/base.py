...
class PanProvider(ABC):
    ...
    def refresh_play_url(self, play: PlayUrl, *, headers: dict[str, str]) -> PlayUrl | None:
        params = dict(play.request or {})
        if play.file_id and not params.get('fileId'):
            params['fileId'] = play.file_id
        return self.resolve_play_url(params, headers=headers, refresh=True)
