...
- **mpv 播放体验**：自动连播、续播、Anime4K 实时超分（三档位，支持快捷键循环切换）、截图、弹幕（弹弹play 匹配转 ASS 装载，设置内默认关闭可开）；mpv 右键菜单已中文化。指定本机任意 mpv 构建即享受与内置完全一致的全功能体验；也可指定 VLC / PotPlayer 等外部播放器接管播放——起播与整季连播可用（播放列表照常加载），但超分、快捷键、截图与进度回传等 mpv 专属能力不可用。
...
- [x] 弹幕自动加载（弹弹play 匹配 → ASS → mpv；默认关闭，无透明度/屏蔽词等产品化项）
- [x] TVBox 兼容性基础能力（全量回归与 FongMi 契约审计进行中）
- [ ] macOS / Linux 打包验证与安装后冷启动验收
- [ ] 弹幕产品化（透明度 / 遮挡 / 屏蔽词 / 倍速时长修正）
...
| **规划中的功能** | **一起看**（多人同屏共播）已在主进程实现 SyncPlay 协议客户端（`syncplay-client.js`，连接 syncplay.pl 服务器）与 DLNA 投屏模块（`dlna-caster.js`），但渲染层界面入口尚未开放，正常使用不触发相关网络请求；产品化上线时会同步更新本节。**弹幕**已实装（默认关闭），匹配与加载经 YuKi 后端 `/danmaku` 中转弹弹play API，不引入额外第三方 SDK。 |
