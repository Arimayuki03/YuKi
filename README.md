...
- **mpv 播放体验**：自动连播、续播、Anime4K 实时超分（三档位，支持快捷键循环切换）、截图、智能跳过片头/片尾（`Shift+O`/`Shift+E` 登记，同片名复用）；mpv 右键菜单已中文化。指定本机任意 mpv 构建即享受与内置完全一致的全功能体验；也可指定 VLC / PotPlayer 等外部播放器接管播放——起播与整季连播可用（播放列表照常加载），但超分、快捷键、截图与进度回传等 mpv 专属能力不可用。
...
环境要求：Node.js 20+、Python 3.14。二进制资源按需就位：Anime4K 着色器随 `npm install` 自动下载；ffmpeg 缺失时应用启动后自动下载补齐；mpv 可在「组件状态」一键下载（开发模式也可先手动放置）；aria2c 无自动下载路径，需自行准备。构建安装包时会经 `scripts/binaries.lock.json` 自动下载并校验所需资源。
...
|---|---|
| `npm start` | 启动开发实例 |
| `npm run test:all` | 完整回归（Python 测试 + JS 单测 + JS 语法门禁 + ESLint/Ruff） |
...
- [x] 弹幕（mpv 播放时可选加载：默认关闭的「自动加载弹幕」开关 + 用户自填的 DanDanPlay AppId/AppSecret 两者齐备后，按片名+集数请求弹弹play API 匹配弹幕并叠加；未填凭据或未开启开关均不发起弹幕网络请求）
- [x] TVBox 兼容性基础能力（全量回归与 FongMi 契约审计进行中）
- [ ] macOS / Linux 打包验证与安装后冷启动验收
- [ ] 一起看（多人同屏共播；主进程协议客户端与 IPC 已就绪，差渲染层 UI）
...
| 事项 | 说明 |
|---|---|
| **数据存储** | 所有个人数据（收藏、历史、观看统计、配置、本地文件索引、日志）仅保存在本机：`%APPDATA%/yuki`（Electron `userData`）与 `~/.yuki/`。不上传至开发者服务器，无云端账号体系。可随时在“设置 → 缓存”或文件管理器中查看/清理。如通过环境变量 `YUKI_DATA_DIR` 显式指定数据目录，以上 `~/.yuki/` 路径相应替换。 |
...
| **规划中的功能** | 后续版本计划加入**一起看**（多人同屏共播）功能。届时会引入新的第三方服务依赖（联播信令/中继服务），相关出站请求与数据处理方式将在功能落地时同步更新至本节。**弹幕**已作为可选功能上线：默认关闭的「自动加载弹幕」开关，且需用户在设置中自行填入 DanDanPlay AppId/AppSecret（应用不内置凭据）；两者齐备后，mpv 播放时会按片名+集数请求弹弹play API（`api.dandanplay.net`）加载弹幕，任一前提不满足则不发起弹幕网络请求。**一起看**的主进程 SyncPlay 协议客户端（`syncplay-client.js`）与 DLNA 投屏模块（`dlna-caster.js`）已就绪，但渲染层界面入口尚未开放，正常使用不触发相关网络请求。 |
