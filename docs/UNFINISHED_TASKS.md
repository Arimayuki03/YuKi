...
## 2026-08-17 本轮已完成的仓库内事项

以下是对本清单执行后的收口结果；它们不再作为“未实现”重复处理，但仍保留需要外部环境的验收项：

- TVBox Phase A–E 的代码主体、行为回归、分层诊断和契约审计已完成；详见 [TVBOX_COMPAT_PLAN_REMAINING.md](TVBOX_COMPAT_PLAN_REMAINING.md)。
- `npm run test:py`、`npm run test:jsunit`（206/206）、`npm run test:js`（40/40）、ESLint（0 错误）和 Ruff 已通过。
- `electron-updater` 已接入打包模式检查链路并写入 `package-lock.json`；发布仓库配置、签名和 tag 发布仍待确定。
- 已生成 [DATA_MAP.md](DATA_MAP.md) 和 [TVBOX_CONTRACT_GAPS.md](TVBOX_CONTRACT_GAPS.md)。
...
| `docs/TVBOX_CONTRACT_GAPS.md` 落盘并拆出高影响项 | 已落盘，待逐项纳入实施 | [TVBOX_COMPAT_PLAN_REMAINING.md](TVBOX_COMPAT_PLAN_REMAINING.md) |
...
| `scripts/build-python.js` 按锁文件安装 | 已改造，待实际构建复核 | [IMPROVEMENT_PLAN.md](../IMPROVEMENT_PLAN.md) |
