...
## 当前行为（2026-08 实现；2026-09 安全语义修订）

| 操作 | 行为 | 数据丢失场景 |
|---|---|---|
| 同步到云端（含定时自动同步） | 本地全量快照 PUT 覆盖云端。**0.2.5 起**上传侧排除敏感键（bangumiToken、dandanAppSecret 等不进云端快照） | B 设备定时器触发时，用旧数据覆盖 A 刚上传的新数据 |
| 从云端恢复（含启动时自动恢复） | 云端快照覆盖本地（覆盖前有本机备份）。**0.2.5 起（P2-5）**恢复落盘收敛为**显式允许表**（64 个纯数据键，默认拒绝）——云端快照携带的键不在表内即被丢弃，`lastConfigUrl` 等链接类键不再可被云端改写 | 本地未同步过的独有修改被清掉 |
| 子开关 | 控制哪些数据文件参与传输 | 不涉及冲突判定 |

已有但与合并无关的安全垫：恢复前自动备份到本机设置键 `webDavRestoreBackup`；「测试连接」可提前发现配置错误。

> **实现注意（2026-09）**：双向合并落地时，写入路径同样必须收敛到 0.2.5 的「上传排除表 + 恢复允许表」语义（`kazumi.js` `WEBDAV_SETTINGS_EXCLUDE` 与 settings 白名单），不得回退为整文件无差别覆盖。
