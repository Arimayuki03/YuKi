...
| **直链播放** | 粘贴一个视频链接（http/https/rtmp/rtsp）直接播放 |
| **下载管理** | aria2c / ffmpeg 下载任务列表 |
| **本地文件** | 白名单目录内的本地媒体浏览与播放 |
| **设置** | 外观 / 播放 / 快捷键 / 下载 / 缓存 / CatVod 源设置 / Kazumi 规则 / 同步 / 系统 / 关于 |
...
- **外部播放器**：可在「设置 → 系统 → 组件状态」指定播放器接管播放，行为按所选类型分流：
  - **指定 mpv**（本机已装的任意 mpv 构建）：自动转为**内置引擎**，体验与内置 mpv 完全一致——原生队列连播、Anime4K 超分、快捷键与右键菜单、截图、续播进度、观看统计全部可用（仅跳过 YuKi 的 OSD 字体等外观注入，保留该播放器自身配置风格）；
  - **指定 VLC / PotPlayer**：作为外部主播放器。实测两者均能正常起播，整季播放列表会写成 `.m3u` 交给播放器原生列表，**连播可用**（播放器内切集即逐集取流）。但 mpv 专属能力全部不可用：Anime4K 超分、应用内快捷键与右键中文菜单、截图、续播进度、弹幕均不生效——进度无法回传，历史条目进度显示 0%，观看统计按播放器运行时长（墙钟）计入。想要完整体验请指定 mpv。
...
- 下载完成弹系统通知，可点击跳转下载页；可在「设置 → 下载」关闭「下载完成通知」。

### 数据与同步

- **观看统计**：记录播放时长与次数（我的页面展示）。
- **收藏**：想看 / 在看 / 看过。
- **Bangumi 同步**：登录 Bangumi 账号后，收藏与观看进度可自动双向同步（评分不参与同步）；进度上报默认关闭，需在「设置 → 同步 → Bangumi 同步」开启。
- **WebDAV 备份**：把收藏、历史、观看统计、规则与应用设置同步到自己的 WebDAV 服务器（坚果云 / Nextcloud 等），支持定时自动同步、「启动时自动恢复」与手动「从云端恢复」。
...
**组件就绪**：应用依赖 mpv（播放）、aria2c（下载）、ffmpeg（HLS 下载）三个外部二进制，安装包已内置。「设置 → 系统 → 组件状态」可查看就绪情况；mpv 缺失可点「下载内置播放器」一键补装，或在「指定播放器」中选择本机任意 mpv 构建（自动转为内置引擎、体验一致）或 VLC/PotPlayer（外部模式）。
...
2. **着色器与字体**：Anime4K 着色器启动时自动下载补齐（带 sha256 校验）；MiSans 字体已随安装包内置，无需下载。
...
### 第二步：添加 Kazumi 规则（番剧源）

三个默认 Kazumi 规则（7sefun / DM84 / enlie）开箱即用，但更多番剧源要靠「规则商店」：

- 进入 **设置 → Kazumi 规则 → 规则商店**，从在线仓库（KazumiRules）浏览、一键安装、更新规则；
- 商店默认按「GitCode 镜像优先、GitHub 备用」拉取，通常无需额外配置即可使用；
- **没有访问外网的环境**（GitHub 不可达时），先去 **设置 → 系统 → 网络** 打开「规则仓库镜像」开关，强制全部走 GitCode 镜像；同页的「Bangumi 镜像」控制番剧封面、评分、收藏同步等 Bangumi 接口，同样建议开启（第三方社区镜像，技术上可见你的请求与 Token，介意隐私可保持关闭）；
- 也可以手动导入：粘贴规则 JSON 或 `kazumi://` 分享链接（见下文「导入规则」）。
...
按「首次启动配置 → 第三步」配置对应网盘的凭据。夸克建议直接扫码登录。Cookie 会过期，过期后重新粘贴或重新扫码。
...
**Q：播放进度显示 0%？**
使用 VLC / PotPlayer 等非 mpv 外部播放器时播放器不回传播放位置，历史条目进度显示 0%，观看统计按播放器运行时长计入，Anime4K 超分、快捷键、截图等 mpv 专属功能也不可用；连播不受影响（整季列表照常加载）。指定 mpv（即使是本机另行安装的）则享受与内置完全一致的完整体验。

**Q：弹幕为什么没有？**
弹幕是真实功能，但有双重前提：①「设置 → 播放」中的「自动加载弹幕」开关（默认关闭）；②「设置」中已填入 DanDanPlay 的 AppId/AppSecret（应用不内置凭据，需用户自行申请后填入）。两者齐备后，mpv 播放起播时会按片名+集数请求弹弹play API（`api.dandanplay.net`）匹配并加载弹幕，以 ASS 字幕形式叠加到画面；匹配不到或请求失败会静默跳过，不影响播放。任一前提不满足则不会发起弹幕网络请求。注意：外部播放器（VLC/PotPlayer）模式下弹幕不生效。
...
| drpy 规则 | ❌ 不支持（需要独立 drpy 运行时，源会被标记 unsupported） |
...
也可以点「规则商店」从在线仓库（KazumiRules）浏览、安装、更新规则。商店拉取默认按「GitCode 镜像优先、GitHub 备用」容错；GitHub 不可达的网络环境，可在「设置 → 系统 → 网络」打开「规则仓库镜像」强制全部走 GitCode 镜像（见「首次启动配置 → 第二步」）。
