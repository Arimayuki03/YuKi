...
9. **一起看（SyncPlay）**：多人同步播放。✅ 已实现（SyncPlay 协议客户端）
10. **DLNA 投屏**：投屏到电视。✅ 已实现（UPnP SSDP + SetAVTransportURI）

### 低优先级（已全部完成 ✅）
11. **外部播放器**：调用系统播放器（VLC 自动探测 + 自定义路径 + Referer 注入）。✅ 已实现
12. **画中画**：桌面 mini 窗（无边框置顶 320x180，可调整大小）。✅ 已实现
13. **定时关机**：播放完成后关机（N 分钟倒计时，停 mpv 后系统关机）。✅ 已实现
14. **日志查看器**：分页查看应用日志（每文件最后 200 行，每页 50 行）。✅ 已实现
15. **首次引导**：新用户向导（欢迎页 + 快速上手指南，确认后不再弹出）。✅ 已实现
