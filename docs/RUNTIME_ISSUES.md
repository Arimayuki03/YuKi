...
> 来源：后台运行日志 `yuki/session-logs/`（2026-08-09 起）。2026-09 起，用户可感问题的主要记录随版本进入 [CHANGELOG.md](../CHANGELOG.md)，本文件保留 2026-08-09～08-26 期间的详细问题档案（R1–R28）与批次级验证结论。
> 本文件只维护运行异常、日志证据、代码修复和复测结果。当前待办汇总见 [../PROGRESS.md](../PROGRESS.md)。

## 当前结论

> 「编号」列 R1–R28 对应下方详情档案的问题编号；G0/S1/C2 行为批次级验收结论，不占问题编号。
...
| G0 | G0 运行时契约、站点健康和兼容夹具退出边界（批次结论） | ✅ 已验证：离线 4 夹具、Python 全量、Node 222/222、JS 语法 40/40 |
| R10 | 以图搜番 URL 直传被拦（trace.moe 需原始上传） | ✅ 已验证：原始文件上传 200 + 10 条结果 |
| G0/S1 | S1 不可信运行时隔离、硬超时、聚合取消和熔断恢复（批次结论） | ✅ 已验证：Python 24 阶段、Node 225/225、JS 语法 40/40、Ruff/ESLint 0 error |
| G0/C2 | C2 配置三层分离、prepare→validate→atomic swap、`ext` 语义、能力路由与配置安全边界（批次结论） | ✅ 已验证：`run_all.py` 28 阶段全通过、编译 79 文件 0 error；四个新阶段共 157 条全部走 loopback 夹具不出网 |
...
- **状态**：✅ 已修复并验证（后端测试通过；0.2.x 起经多版本真实用户收藏同步使用未再复现）。
...
> **外部播放器能力边界（用户口径小结，2026-09-22 实测定版）**：「指定播放器」按所选类型分流——选中 **mpv**（本机任意构建）自动转为内置引擎，体验与内置完全一致（原生队列连播、Anime4K 超分、快捷键与右键菜单、截图、续播进度、观看统计全可用，仅跳过 OSD 字体等外观注入）；选中 **VLC / PotPlayer** 则为外部主播放器——起播正常，整季播放列表（本地 `.m3u` + 按需解析代理条目）能被正常加载，**连播可用**（播放器内切集即逐集向本地代理取流，直链零过期特性不变）。但 mpv 专属能力全部不可用：Anime4K 超分、应用内快捷键与右键中文菜单、截图、续播进度与弹幕均不生效；播放器不回传播放位置，历史条目进度显示 0%，观看统计按播放器运行墙钟计入（ext-watch 会话，≥15s 记一次）。想要完整体验请指定 mpv。
...
### R28 · 夸克网盘「转存失败」——Chrome UA 过不了写操作风控 + 失败重试风暴把临时风控打成持续 403（2026-08-26）

- **发现时间**：2026-08-26（用户报告：夸克分享源播放报「转存失败」；同一账号在手机 TVBox 上转存正常）
- **根因**（两层叠加）：
  1. **接口身份错误**：`sharepage/save` 写操作按客户端签名做风控——普通 Chrome UA 读接口（token/v2play）可过、save 必回 HTTP 403；手机 TVBox 的 JAR 自带 PC 客户端 UA 故转存正常。
  2. **重试风暴**：转存失败被当普通瞬时错误进入 1–2s 间隔重试循环，渲染层自动线路回退再对同一分享打一轮完整链路（token → v2/play → download → detail → sharepage/save），几秒内十余次密集 save 被夸克打成持续 403。
- **修复**：
  - `go_proxy.py`：新增 `QUARK_API_UA`（夸克 PC 客户端 UA，AList/OpenList/gopeed/quark-auto-save 社区事实标准），接口请求与后续 CDN 取流统一同一身份（直链签名与请求身份绑定，混用 UA 会复现 412），补 `Origin` 头；分享接口域名 `drive.quark.cn` → `drive-pc.quark.cn`；会话保活探针同步换 UA。
  - `_QuarkSaveDenied` 语义化拒绝：HTTP 401 或 403 且业务码 ≠41020 判不可自愈硬拒绝，立即上抛跳过剩余重试与个人盘兜底（文件未转存成功时兜底只会多打两轮 v2/play/download 加深风控）；code=41020「转存文件token校验异常」虽以 HTTP 403 包装，但属 JAR vodId 快照令牌过期的**可自愈**错误——按业务码区分而非只看 HTTP 状态，调用方拉实时目录树换新令牌重转存即可成功。代理侧回 502 + 中文提示「今日转存次数已达上限或触发风控」，排查时可直接定位账号额度/风控而非链路故障。
  - `src/main/pan-source.js`（新增）+ `player.js`：网盘类源（站点/线路名或集地址命中 `PAN_SOURCE_RE`，Kazumi 规则引擎豁免防误伤）禁用自动线路回退（失败即收口交用户手动重试）、全局禁用原生播放列表（主进程拒建队 → 渲染层静默回退逐集连播）——网盘解析慢且依赖 Cookie 会话，整季装载只会放大解析次数与风控暴露面。
- **验证**：`python-backend/tests/test_quark_pan.py` 24/24 OK（覆盖 `_QuarkSaveDenied` 不重试、41020 自愈重转存、快路径分流等）；`tests/js/pan-source-playlist.test.js` 7/7（建队拦截、集地址协定命中、Kazumi 豁免、主/渲两端正则同源、网盘源回退风暴钉死）。真实账号端到端复测待用户验证。
