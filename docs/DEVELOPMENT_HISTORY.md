...
55. **ffmpeg 内置化**（决策 72）：m3u8 合成与本地预览图共用；启动 ensureFfmpeg 幂等下载 ffmpeg 官方构建（2026-09-22 起为 BtbN/FFmpeg-Builds 版本化资产，约 190MB；原 gyan.dev essentials 包已下线）→ vendor/ffmpeg，失败静默降级、其次探测 PATH；缩略图 5s 处抓帧缩 480 宽 jpg，md5(路径|mtime|大小) 缓存 userData/local-thumbs，并发 4。
