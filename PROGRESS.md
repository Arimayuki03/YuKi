...
- 外部播放器（VLC / PotPlayer）能力边界（实测确认）：起播正常，整季播放列表写成 `.m3u` 交播放器原生列表，**连播可用**（本地按需解析代理逐集取流，直链零过期不变）；但 mpv 专属能力全部不可用——Anime4K 超分、应用内快捷键与右键菜单、截图、续播进度与弹幕不生效，进度无法回传（历史条目进度 0%），观看统计按播放器运行墙钟计入（`ext-watch` 会话追踪，≥15s 记一次）。
