...
> 行号锚点目录（编辑后若漂移，grep `^##` 重新定位）：§1 项目概述 L9 · §2 Phase 总览 L17 · §3 环境与命令 L33 · §4 架构决策 L57（子节：进程通信 L59 / 插件爬虫 L66 / 前端UI L77 / 文件管理 L86 / 下载 L91 / 推送设置解析 L99）· §5 Spider 契约 L130 · §6 构建打包 L145 · §7 前端注意与功能 L165（注意事项 L167 / 功能概览 L177）· §8 已知坑位 L188 · §8.7 待完成 L208
...
7. **不要重新引入已回滚功能**：视频缓存到硬盘切换（--cache-on-disk）、直播源后台探测（保持阻塞式探测于 loading 内）、弹幕功能。
...
- **直播**：mpv 健康检测备用线路（U2/36）、TVBox 式导入（U65/52）、频道可用性阻塞探测（FEATURES#4）、IDN punycode（U36）。
