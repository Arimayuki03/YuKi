...
- 外部播放器能力边界（实测确认）：「指定播放器」按类型分流——选中 mpv（本机任意构建）经 `setCustomPath` 转为内置引擎，体验与内置完全一致（连播/超分/快捷键/截图/续播/统计全可用，仅跳过 OSD 字体等外观注入）；选中 VLC / PotPlayer 则起播正常，整季播放列表写成 `.m3u` 交播放器原生列表，**连播可用**（本地按需解析代理逐集取流，直链零过期不变）；但 mpv 专属能力全部不可用——Anime4K 超分、应用内快捷键与右键菜单、截图、续播进度与弹幕不生效，进度无法回传（历史条目进度 0%），观看统计按播放器运行墙钟计入（`ext-watch` 会话追踪，≥15s 记一次）。
...
- 弹幕产品功能当前关闭。DanDanPlay API、ASS 生成和历史端点虽仍存在，但产品未启用弹幕入口，不应把“补弹幕渲染”当作当前缺陷。
...
[docs/THIRD_PARTY.md](docs/THIRD_PARTY.md)（mpv GPLv2+ / ffmpeg BtbN 构建 GPLv3 / aria2c GPLv2 /
